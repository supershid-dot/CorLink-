...
- **One or more inbound edges are required** (`gateway_inbound_edges_invalid` rejects only zero), matching docs/69 exactly: "one or more are structurally permitted... reusing exactly the same rule and the same proof obligation docs/63 already states for approval nodes." This represents ordinary reconvergence under the Version 2 single-token model — because at most one predecessor path can ever be live in a given execution (no split exists), multiple structural inbound edges, whether from one upstream node or several, are always safe; only *zero* inbound edges (an unreachable node) is invalid. This is not a synchronizing merge — no runtime object, token, or state waits for more than one arrival; see "Merge behavior" below. Split/join and multi-token behavior remain deferred to a future capability version, per docs/69. (Corrected by CAP-002 Phase 4.1A after an earlier draft of this milestone incorrectly required exactly one inbound edge, mirroring the approval node's stricter — and unrelated — precedent; see "Limitations".)
...
Multiple gateway (and/or approval) outbound edges may target the same downstream node — ordinary graph convergence, requiring no new validation concept beyond the corrected one-or-more inbound-edge rule above, verified as a dedicated behavioral scenario. Because a `gateway_exclusive` node requires only at least one inbound edge (not exactly one), reconvergence is supported onto a downstream `gateway_exclusive` node as well as onto `end` — a graph where two or more upstream gateway branches converge on a second gateway node publishes successfully, since the single-token model already guarantees only one of those paths is ever actually live. Reconvergence onto a downstream `approval` node remains unsupported, because the `approval`-node inbound rule is unrelated to this correction and is unchanged: an approval round is stateful (a quorum snapshot opens fresh at entry), so entry ambiguity there is a real correctness concern that does not apply to the stateless `gateway_exclusive` node.
...
- **Gateway reconvergence is available onto `end` nodes and onto a second `gateway_exclusive` node, but not onto an `approval` node.** A `gateway_exclusive` node permits one or more inbound edges (only zero is rejected), matching docs/69 exactly — this represents ordinary reconvergence under the single-token model, not a synchronizing merge; no runtime object waits for more than one arrival. The `approval`-node inbound rule is unrelated and unchanged: it still requires exactly one inbound edge, because an approval round is stateful (a quorum snapshot opens fresh at entry) in a way a stateless `gateway_exclusive` node is not. *(Corrected by CAP-002 Phase 4.1A: an earlier draft of this milestone incorrectly required exactly one inbound edge for `gateway_exclusive` too, by over-literally reusing the approval node's code pattern rather than its underlying rationale. That draft was never pushed or approved; the correction is folded directly into this document and the Phase 4.1 patch.)*
- **Split, join, and multi-token execution remain fully deferred**, per docs/69's own Version 2 scope. Permitting multiple inbound edges into a `gateway_exclusive` node is ordinary graph convergence, not a synchronizing merge — no token identity, parent/lineage, or join-arrival tracking is introduced by this correction or by this milestone.
