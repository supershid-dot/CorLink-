...
| **(Recurring meetings Phase 2 — SHIPPED including frontend, see row above; any org-level Rooms/Meetings configuration remains pending)** | Recurring Meetings Phase 2 (this/future/all-series edit and cancel, exception dates, skipped occurrences) shipped, backend and frontend. **Update, 2026-07-24: the Phase 2 frontend UI — the scope-selection dialog, shared Entire-Series/This-and-Future edit and cancel modals, the reusable result-summary modal, and a read-only series activity timeline — has since shipped**, plus a follow-up backend fix closing a meeting-series audit-visibility gap; see `docs/28-recurring-meetings-phase2-implementation.md` §16/§17 for the full record. A staging migration rehearsal has since validated Phase 2 (and the rest of the Platform Migration/Rooms/Meetings chain) end to end; see `docs/29-recurring-meetings-phase2-production-deployment-runbook.md` for the operator runbook and the outstanding multi-persona UAT gap. The optional org-level working-hours configuration noted in §3.4 remains unbuilt. | Phase 2's RPC surface shipped first, ahead of its dedicated frontend, which has since also shipped (see the Update note in the cell to the left) — this row is no longer "backend only." The working-hours configuration remains deferred with no scheduled phase. None of Phase 2 (backend or frontend) has yet been applied to production Supabase — staging-validated only. |
