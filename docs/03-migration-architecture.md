...
- **Staging migration** — Phases 1–13 (§8) are first executed and validated in full before any staff-facing frontend exists (Phase 14 comes after). This section originally assumed no separate staging Supabase project would be introduced, with local-Postgres RLS verification serving as the sole pre-production check. **Update, 2026-07-24: a real staging environment has since been used to rehearse the full Platform Migration/Rooms/Meetings/Recurring/Drafts SQL chain**, superseding that original assumption — see `docs/29-recurring-meetings-phase2-production-deployment-runbook.md` §2 for the rehearsal evidence and §1/§3 for what staging validated versus what remains a production-readiness gap. Local-Postgres RLS verification remains the standard pre-production check for every individual RLS-touching change, in addition to the staging rehearsal.
