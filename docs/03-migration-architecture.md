...
| Recurring meetings | **Later (server-side redesign required first)** | MeetFlow materializes up to 52 rows via a client-generated `recurrence_id` — a client-trusted pattern CorLink does not use anywhere else. V1 ships single (non-recurring) meetings only; recurrence ships once a `SECURITY DEFINER` RPC replaces the client loop | Meetings (Later) | Medium — must not repeat MeetFlow's client-trusted generation pattern | Meetings core stable in production |
...
- **`meetings`** — `id`, `org_id`, `section_id` (FK to canonical `sections`), `title`, `type` (internal/external), `meeting_mode` (physical/online/both), `meeting_link` (scheme-validated on write — closes the `javascript:`-URI gap noted in the audit), `privacy` (confidentiality-aware, feeds §7's RLS design), `room_id` (nullable FK to `meeting_rooms`), `created_by` (FK to `users`), `is_cancelled`/`cancelled_at`/`cancelled_reason`, `is_locked`, `recurrence_id` (nullable, Later — §2), `minutes_finalized`. **Scheduling fields are `start_at TIMESTAMPTZ`, `end_at TIMESTAMPTZ`, `timezone TEXT`** — not MeetFlow's `date`+`start_slot`(15-minute index)+`duration`(slot units). See conversion note below.
- **`meeting_participants`** — `id`, `meeting_id`, `user_id` (nullable — null for external), `rsvp_status`, `attendance_status`, `attendance_marked_at`, `attendance_marked_by`.
- **External participants** — modeled as rows in `meeting_participants` with `user_id IS NULL` plus `external_name`/`external_email` columns (not a separate table) — mirrors how CorLink already handles polymorphic participant-like concepts elsewhere (e.g. `cc_recipients`), and keeps RSVP/attendance tracking unified across internal and external attendees.
...
### Phase 3 — Meeting schema
- **Purpose:** Create `meetings`, `meeting_participants`, `meeting_agenda_items`, `meeting_decisions` per §5.
- **Prerequisites:** Phase 1 complete (module-gated RLS depends on it).
- **Changes:** New tables + RLS (§7) + helper functions as needed; zero existing tables altered.
- **Validation:** Local Postgres RLS verification, per this repo's established convention (stub `auth` schema, real `schema.sql`+`rls.sql`, hex-only UUID seed data) — same process used for every RLS-touching change this session.
