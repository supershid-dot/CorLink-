...
| Phase F (Recurring/Drafts) | Phase C (Calendar) | Calendar must render draft/recurring items from day one (Q5). **Partially honored — Recurring Meetings Phase 1 had shipped first, so recurring occurrences render; Draft/Pre-booked Meetings never shipped, so Calendar has no draft-slot data source. See `docs/26` §2/§3.** |
...
7. **Then:** Phase C (Calendar) — requires F and H both complete. **As shipped, Calendar went ahead after F alone, with H's leave data and F's Draft/Pre-booked Meetings both omitted from scope rather than waited on — see `docs/26`.**
