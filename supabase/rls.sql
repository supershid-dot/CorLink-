...
-- ─── cc_recipients (CC / Loop-In Staff, like CC in email) ───────
...
-- Section" (internal_requests, above), which routes a whole section
-- extra info to gather and reply to. A CC just gives one named person
-- a read-only window onto the exact artifact they were CC'd on.
--
...
-- ─── approvals ────────────────────────────────────────────────
-- Previously: reviewed_by/is_admin()/record creator only — which hid
-- an approval row (and therefore the "approved by [Supervisor]" banner
-- in request-detail.js's _renderApprovalHistory) from EVERYONE else,
-- including a receiving-org supervisor or section member who isn't the
-- request's own creator, and including the request/response's own
-- to-org section (only the literal creator qualified). is_admin() was
-- also too broad in the other direction — ANY org's admin, unrelated
-- to this record's own orgs. Rewritten to mirror requests_select/
-- responses_select's exact visibility shape: whoever can see the
-- parent request/response can see its approval history too (the
-- _renderApprovalHistory ownSide filter is what actually hides
