-- ============================================================
-- CAP-002 Phase 2B.2: Executable Workflow Instance Activation
--
-- Implements docs/63's "Instance activation contract" for
-- schema-version-1 instances on top of the inert Phase 1 foundation,
-- the Phase 2 runtime lifecycle, and the Phase 2B.1 executable
-- definition validator. Extends the SHARED workflow_transition_
-- instance's 'start' command only; legacy inert instances (pinned to
-- a version whose payload has no 'schema_version' key) receive
-- byte-for-byte the exact same single-event start behavior Phase 2
-- already gave them. suspend/resume/cancel/complete are completely
-- untouched.
--
-- Scope boundary (see docs/65 for the full reasoning): activation
-- enters the Start node, moves the token through its sole 'started'
-- edge, and enters exactly one first executable node (Approval or
-- End). For Approval, it resolves candidates and creates the round/
-- positions/work items required to represent entry (docs/63's own
-- "Phase 2B... create initial token, steps, round, participants, and
-- work items") but records no decision and completes nothing. For
-- End, it completes that node and the instance atomically — an
-- explicit, bounded, docs/63-authorized special case ("If it is end,
-- complete it and the instance"), not generic graph advancement.
-- Nothing beyond this first node is entered, decided, or advanced.
-- ============================================================

BEGIN;

-- ── 1. Approval round/position persistence (Phase 2B.1 deferred
--    this DDL to "that implementation" — this is it) ───────────────

CREATE TABLE IF NOT EXISTS workflow_approval_rounds (
  ...
  delivery_mode       TEXT        NOT NULL CHECK (delivery_mode IN ('sequential','parallel')),
  decision_rule       TEXT        NOT NULL CHECK (decision_rule IN ('unanimous','majority')),
  requirement         TEXT        NOT NULL CHECK (requirement IN ('required','optional')),
  optional_policy     TEXT        CHECK (optional_policy IS NULL OR optional_policy = 'skip_if_no_candidates'),
  minimum_candidates  INTEGER     NOT NULL CHECK (minimum_candidates BETWEEN 1 AND 100),
  electorate_count    INTEGER     NOT NULL CHECK (electorate_count BETWEEN 0 AND 100),
  approval_threshold  INTEGER     NOT NULL CHECK (approval_threshold >= 0),
  reject_behavior     TEXT        NOT NULL CHECK (reject_behavior IN ('immediate','when_approval_impossible')),
  allow_abstain       BOOLEAN     NOT NULL,
  comment_policy      JSONB       NOT NULL CHECK (jsonb_typeof(comment_policy) = 'object'),
  state               TEXT        NOT NULL DEFAULT 'open' CHECK (state IN ('open','completed','cancelled','failed')),
  outcome_code        TEXT        CHECK (outcome_code IS NULL OR outcome_code IN ('approved','rejected','skipped','cancelled','failed')),
  ...
  CONSTRAINT workflow_approval_rounds_step_unique UNIQUE (step_id),
  CONSTRAINT workflow_approval_rounds_state_alignment_check CHECK (
    (state = 'open' AND outcome_code IS NULL AND completed_at IS NULL)
    OR (state <> 'open' AND completed_at IS NOT NULL)
  )
);

CREATE INDEX IF NOT EXISTS idx_workflow_approval_rounds_instance
  ON workflow_approval_rounds (instance_id, state, created_at, id);

DROP TRIGGER IF EXISTS set_updated_at ON workflow_approval_rounds;
CREATE TRIGGER set_updated_at BEFORE UPDATE ON workflow_approval_rounds
  FOR EACH ROW EXECUTE FUNCTION trigger_set_updated_at();

CREATE TABLE IF NOT EXISTS workflow_approval_positions (
  id                  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  instance_id         UUID        NOT NULL REFERENCES workflow_instances(id),
  round_id            UUID        NOT NULL REFERENCES workflow_approval_rounds(id),
  step_id             UUID        NOT NULL REFERENCES workflow_instance_steps(id),
  ...
  state               TEXT        NOT NULL DEFAULT 'pending' CHECK (state IN ('pending','offered','decided','cancelled','unavailable')),
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT workflow_approval_positions_round_key_unique UNIQUE (round_id, position_key),
  CONSTRAINT workflow_approval_positions_round_ordinal_unique UNIQUE (round_id, ordinal),
  CONSTRAINT workflow_approval_positions_work_item_unique UNIQUE (work_item_id),
  CONSTRAINT workflow_approval_positions_decision_unique UNIQUE (decision_id)
);

CREATE INDEX IF NOT EXISTS idx_workflow_approval_positions_round
  ON workflow_approval_positions (round_id, ordinal);
CREATE INDEX IF NOT EXISTS idx_workflow_approval_positions_user_state
  ON workflow_approval_positions (user_id, state, round_id);

ALTER TABLE workflow_approval_rounds    ENABLE ROW LEVEL SECURITY;
ALTER TABLE workflow_approval_positions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS workflow_approval_rounds_select ON workflow_approval_rounds;
CREATE POLICY workflow_approval_rounds_select ON workflow_approval_rounds
  FOR SELECT TO authenticated
  USING (can_view_workflow_instance(instance_id));

DROP POLICY IF EXISTS workflow_approval_positions_select ON workflow_approval_positions;
CREATE POLICY workflow_approval_positions_select ON workflow_approval_positions
  FOR SELECT TO authenticated
  USING (can_view_workflow_instance(instance_id));

REVOKE ALL ON TABLE workflow_approval_rounds, workflow_approval_positions FROM PUBLIC, anon, authenticated;
GRANT SELECT ON TABLE workflow_approval_rounds, workflow_approval_positions TO authenticated;

-- ── 2. workflow_transition_instance — 'start' extended for
--    schema-version-1 instances only; every other command and the
--    legacy-inert 'start' path are byte-for-byte unchanged. ────────
