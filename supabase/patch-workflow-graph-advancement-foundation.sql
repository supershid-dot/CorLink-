...
--
-- Implements docs/63's "Generic graph-advancement contract" for
-- executable-v1 instances on top of the inert Phase 1 foundation, the
-- Phase 2 runtime lifecycle, the Phase 2B.1 executable definition
-- validator, the Phase 2B.2 instance activation checkpoint, and the
-- Phase 2B.2A event-sequencing correction.
--
-- Extracts the "enter a downstream node" mechanics Phase 2B.2 already
-- proved correct (move the token, create the target step, and either
-- complete an End node and the instance, or resolve candidates and
-- open an Approval round) into one shared, private, generic helper —
-- workflow_enter_downstream_node() — and makes BOTH activation and
-- this milestone's new advancement command call it. Activation's
-- externally observable behavior (events, sequencing, return values)
-- is unchanged; this is a pure extract, not a redesign.
--
-- The new command, workflow_advance_graph_step(), implements docs/63's
-- generic outcome-to-edge routine: given the instance's current token
-- already sitting at a step that already carries exactly one terminal
-- result (set by some other, already-authorized process — nothing in
-- this milestone or any prior one can produce that result for an
-- Approval node, since decisions are Phase 3's job), it selects the
-- one matching outbound edge and calls the same shared helper to
-- enter the resolved downstream node. It never decides, calculates,
-- or supplies an approval outcome itself.
--
-- Scope boundary: version-1 node universe is start/approval/end;
-- 'start' can never be a legal advancement target (zero inbound
-- edges is enforced at publication) and is rejected defensively by
-- the shared helper, not a supported case. No decision, outcome
-- calculation, routing, gateway, condition, timer, notification,
-- delegation, module adapter, or frontend code is added.
-- ============================================================

BEGIN;

-- ── 1. Shared, private downstream-node-entry helper. Never granted
--    to any client role; reachable only from within another
--    SECURITY DEFINER function's already-authorized, already-locked
--    transaction scope (matching canonicalize_workflow_definition_
--    payload's precedent: a private, parameter-driven helper that
--    trusts the caller's already-established authorization boundary
...
-- ── 2. workflow_transition_instance — its 'start' branch now calls
--    the shared helper above instead of carrying its own inline
--    End/Approval entry logic. Externally observable behavior
--    (events emitted, their sequence numbers and metadata, return
--    values) is byte-for-byte unchanged from the approved Phase
--    2B.2/2B.2A baseline — this is a pure extract, not a redesign.
--    Every other command and the legacy-inert 'start' path remain
--    byte-for-byte unchanged, exactly as they already were. ────────
...
-- ── 3. workflow_advance_graph_step — the new Phase 2C.1 command.
--    Given the instance's current token already sitting at a step
--    that already carries exactly one terminal result, selects the
--    one matching outbound edge and calls the same shared helper
--    used by activation to enter the resolved downstream node. Never
--    a client-selected target-node RPC — the current step is always
--    derived from the instance's own sole active token, never
