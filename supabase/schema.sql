...
-- RLS (requests_update_supervisor/responses_update_supervisor in
-- rls.sql) gates WHO can update these rows, but says nothing about
-- WHICH status transition is being made — a legitimately-authorized
...
-- reviewed it and why, and needing no real approval to have happened),
-- or otherwise walk the status column somewhere the app's own workflow
-- never goes. This trigger is the one place that actually enforces the
-- state machine app.js's functions already assume.
--
-- Every edge below is taken directly from js/data/requests-api.js's
-- actual .update({status: ...}) call sites — nothing here is invented.
-- 'overdue' (requests only; set by check_deadlines() in
-- supabase/notifications.sql, which touches any row NOT already in
-- draft/closed/responded/overdue) is an orthogonal flag layered on top
-- of pending_approval/sent/received/in_progress rather than its own
-- branch of the chain — once a row goes overdue, the information
-- about which of those four it actually was is gone, so every
-- downstream edge any of the four could have taken is allowed FROM
-- 'overdue' too, rather than trying to guess which one applies.
...
RETURNS BOOLEAN AS $$
  SELECT old_status = new_status OR (old_status, new_status) IN (
    ...
    ('in_progress', 'overdue'),
    ('responded', 'closed'),
    ('overdue', 'sent'),        -- was pending_approval, then approved
...
-- received_at, matching the CHECK constraint's inclusion of 'received'
-- being unused today. No 'overdue' path exists for responses either
-- (check_deadlines() only touches requests).
