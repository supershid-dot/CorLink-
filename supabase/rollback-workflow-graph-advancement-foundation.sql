-- CAP-002 Phase 2C.1 rollback.
--
-- Restores workflow_transition_instance() to its exact pre-2C.1
-- (Phase 2B.2A-corrected) body — copied verbatim from
-- supabase/patch-workflow-activation-event-sequence-correction.sql
-- (a file this patch never edited, so it remains the authoritative
-- pre-2C.1 source, not hand-transcribed) — and drops the two new
-- functions this patch added, workflow_advance_graph_step() and
-- workflow_enter_downstream_node().
--
-- Unlike the definition/activation rollbacks earlier in this chain,
-- this one never refuses: it adds no table and changes only which
-- function bodies exist and which formula a function uses on future
-- calls. Any instance already advanced via workflow_advance_graph_step
-- has its state fully represented in the unchanged, pre-existing
-- Phase 2B.2 tables (steps, tokens, rounds, positions, work items,
-- events) — rollback does not delete any row and cannot destroy that
-- history; it only removes the ability to advance further until
-- reapplied. There is therefore nothing for a preflight check to
-- protect.
\set ON_ERROR_STOP on
BEGIN;

DROP FUNCTION IF EXISTS workflow_advance_graph_step(UUID,BIGINT,UUID);
