-- 136: Meeting creator is no longer forced into the participant list.
--
-- Until now, create_meeting() unconditionally inserted the caller as an
-- 'organizer' participant. That meant the person scheduling a meeting
-- always showed up as a participant (and as its Organizer) even when
-- they were never actually attending — e.g. an admin/secretary
-- scheduling on behalf of a section. UAT: "when a person creates a
-- meeting, that person may not be a participant of that meeting, so he
-- will not be in the participant list unless he selected as a
-- participant, this feature must be added" — and the follow-up answer
-- on who should show as organizer instead: "whoever the admin
-- explicitly marks as organizer in the participant picker".
--
-- add_participant() already accepts p_participant_role ('organizer'/
-- 'attendee'/'observer') and already rejects a second organizer via its
-- own unique_violation handler ("This meeting already has an
-- organizer") — so once the frontend fully owns participant creation
-- for a new meeting, it can freely designate any one selected
-- participant (which may or may not be the creator) as organizer
-- through that same existing RPC. The only backend gap was
-- create_meeting()'s own unconditional insert always claiming the
-- organizer slot for the creator first.
--
-- Fix: a new trailing parameter, p_include_creator_as_participant,
-- defaulting to TRUE (so every existing caller that doesn't pass it —
-- create_recurring_meeting() from the standalone "New Recurring
-- Series" modal, which has no per-occurrence participant picker at
-- all — keeps today's exact behavior unchanged). The combined Schedule
-- Meeting form (single meeting or its own recurrence quick-picks,
-- js/views/meetings.js) now passes FALSE and applies every participant
-- — including the creator, if explicitly selected — itself via its
-- existing post-creation add_participant() loop.
--
-- Bodies below reproduce the current live functions
-- (supabase/patch-meetings-notification-completion.sql) verbatim, with
-- only the trailing parameter and the new conditional guard added —
-- confirmed against pg_get_functiondef() on CorLink Staging before
-- writing this file.
--
-- Each function's prior signature is DROPped explicitly first — CREATE
-- OR REPLACE cannot change a parameter list, it would just create a
-- second overload alongside the old one (same precedent as
-- patch-meetings-section-scope.sql / patch-meetings-notification-
-- completion.sql's own DROP FUNCTION lines for this exact situation).
...
CREATE OR REPLACE FUNCTION create_meeting(
  ...
  p_include_creator_as_participant BOOLEAN DEFAULT TRUE
)
RETURNS UUID AS $$
DECLARE
...
BEGIN
  ...
  IF p_include_creator_as_participant THEN
...
DROP FUNCTION IF EXISTS create_recurring_meeting(TEXT, DATE, DATE, TIME, TIME, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, UUID, UUID, INTEGER, UUID);
...
BEGIN
  ...
  LOOP
    ...
    v_meeting_id := create_meeting(
      ...
      p_include_creator_as_participant := p_include_creator_as_participant
    );
