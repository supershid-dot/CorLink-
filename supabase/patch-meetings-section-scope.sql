-- ============================================================
-- CorLink — Patch: section-scoped meeting access control
--
-- UAT correction: meetings previously used a blanket
-- is_supervisor_or_above() term (is_admin() OR has_role('supervisor'))
-- for both can_manage_meeting() and can_view_meeting() — ANY user
-- holding a 'supervisor' user_assignments row ANYWHERE in the org
-- could see/edit EVERY meeting in that org, regardless of which
-- command/department/division/section that assignment was actually
-- scoped to. Required behavior instead: a meeting's own section_id
-- (new — meetings had no section reference before this patch, unlike
-- meeting_room_bookings.section_id, which already existed) determines
-- who beyond the creator/participants can act on it —
--   * plain staff of that SAME section, and
--   * a command/department/division head whose own scope EXPANDS to
--     cover that section (via scope_section_ids(), already built —
--     supabase/patch-user-assignments-scope.sql)
-- both get access, and a user holding more than one qualifying
-- assignment (multiple sections/departments/commands) gets the union
-- of all of them for free — my_section_ids() already aggregates every
-- one of a caller's user_assignments rows, at any scope_type, with no
-- extra work needed here.
--
-- This is the EXACT correction supabase/patch-narrow-supervisor-
-- visibility.sql already made for requests_select/responses_select
-- (blanket has_role('supervisor') -> my_section_ids()) — applied here
-- to meetings for the first time. is_admin() (mcs_admin/authority_admin)
-- is deliberately UNCHANGED — those remain org-wide, matching every
-- other module in this codebase; only the narrower 'supervisor' role
-- ever received org-wide reach here, and that was the bug.
--
-- A meeting with section_id left NULL (the default — nothing forces a
-- meeting to be tagged to a section) behaves exactly as before this
-- patch: only creator/participants/org-wide-admin/organization-
-- visibility apply. Only meetings explicitly tagged via "Section"
-- gain the new section/head-scoped access — no regression for
-- existing meetings.
--
-- Locking is deliberately NOT touched: is_meeting_lock_overridable()
-- still only allows the creator, an org-wide admin, or a super admin
-- to override a lock. A locked meeting is not editable by the new
-- section-staff/head grant, matching "meetings can be edited [by
-- section staff/heads] unless it is locked" literally — that
-- exception was already the correct, narrower gate on this codebase's
-- own terms (a plain supervisor could never override a lock even
-- under the OLD blanket rule), so heads/section-staff don't gain that
-- either.
--
...
ALTER TABLE meetings ADD COLUMN IF NOT EXISTS section_id UUID REFERENCES sections(id);
CREATE INDEX IF NOT EXISTS idx_meetings_section ON meetings(section_id) WHERE section_id IS NOT NULL;

ALTER TABLE meeting_series ADD COLUMN IF NOT EXISTS template_section_id UUID REFERENCES sections(id);

-- CREATE OR REPLACE does not replace a function whose parameter list
-- changes shape — a new trailing parameter changes its type-identity,
-- so it creates an ADDITIONAL overload instead of replacing the
-- original, leaving both callable and any named-argument call
-- ambiguous between them. Drop the exact pre-patch signatures first.
DROP FUNCTION IF EXISTS create_meeting(TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS update_meeting(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT, BOOLEAN, BOOLEAN);
DROP FUNCTION IF EXISTS create_recurring_meeting(TEXT, DATE, DATE, TIME, TIME, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, UUID, UUID, INTEGER);
  ...
  SELECT
    ...
    OR EXISTS (
      SELECT 1 FROM meetings m WHERE m.id = p_meeting_id AND (
        ...
        OR (m.organization_id = get_my_org_id() AND is_admin())
        OR m.section_id IN (SELECT my_section_ids())
      )
    );
...
-- ─── create_meeting / update_meeting / create_recurring_meeting: an
-- optional p_section_id, appended last so no existing positional
-- caller breaks. Validated against the caller's own org (informational
-- tag, same as meeting_room_bookings.section_id already is — not
-- restricted to sections the caller belongs to, matching that sibling
-- feature's own existing, unrestricted design) but never a different
-- org's section. ──────────────────────────────────────────────────
