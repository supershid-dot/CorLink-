-- ============================================================
-- CorLink — Validate: Prisoner Letters ↔ Shared Tasks Integration
...
-- the prisoner_letter branch of can_view_task_link(), all new helpers/
-- RPCs, search_path pinning, and that R4's original task_links indexes
-- are still intact. Raises an exception listing anything missing
-- rather than failing silently.
-- ============================================================

-- ─── 1. module_key restriction widened to include 'prisoner_letter' ──
SELECT
  ...
  pg_get_constraintdef(oid) LIKE '%''external_correspondence''%'   AS module_key_still_allows_entry,
  pg_get_constraintdef(oid) LIKE '%''prisoner_letter''%'           AS module_key_now_allows_prisoner_letter
FROM pg_constraint WHERE conrelid = 'task_links'::regclass AND contype = 'c' AND conname LIKE '%module_key%';

-- Confirm a still-unsupported module_key ('room_booking') is rejected —
-- this is genuinely the LAST module_key value this program adds, so
-- there is no next real candidate to reference; any placeholder works.
DO $$
BEGIN
  BEGIN
    INSERT INTO task_links (task_id, module_key, record_id, organization_id, created_by)
    VALUES (gen_random_uuid(), 'room_booking', gen_random_uuid(), gen_random_uuid(), gen_random_uuid());
    RAISE EXCEPTION 'VALIDATION_FAILED: unsupported module_key ''room_booking'' was accepted';
  EXCEPTION
    WHEN check_violation THEN
      RAISE NOTICE 'module_key CHECK correctly rejects a still-unsupported value (''room_booking'')';
...
FROM (VALUES
  ...
  ('create_prisoner_letter_supporting_task'), ('link_existing_task_to_prisoner_letter'),
  ('unlink_task_from_prisoner_letter'), ('list_prisoner_letter_tasks'),
  ('list_task_prisoner_letter_links'), ('get_prisoner_letter_task_capabilities'),
  -- Reused, unmodified R3/R4/R5/R6/R7 functions this milestone depends on:
  ('can_manage_task'), ('can_view_task_link'), ('can_view_task'),
  ('is_prisoner_letters_staff'), ('can_view_entry'), ('can_view_internal_request')
) AS expected(proname)
ORDER BY exists ASC, proname;

-- can_view_task_link's prisoner_letter branch specifically (not just
-- its existence) — confirms the function body was actually widened a
-- fourth time, on top of every prior branch, not replacing any of them.
SELECT
  pg_get_functiondef(oid) LIKE '%can_view_prisoner_letter%'      AS can_view_task_link_has_prisoner_letter_branch,
  pg_get_functiondef(oid) LIKE '%can_view_entry%'                 AS can_view_task_link_still_has_entry_branch,
...
-- can_view_prisoner_letter() mirrors prisoner_letters_select's own
-- conditions — spot-check the is_prisoner_letters_staff/from_prison_id/
-- to_org_id shape is present (not a stand-in that always returns
-- TRUE/FALSE).
SELECT
  pg_get_functiondef(oid) LIKE '%is_prisoner_letters_staff%' AND pg_get_functiondef(oid) LIKE '%from_prison_id%'
    AND pg_get_functiondef(oid) LIKE '%to_org_id%'
    AS can_view_prisoner_letter_has_full_visibility_shape
FROM pg_proc WHERE proname = 'can_view_prisoner_letter';
...
--        shared index — first validator to exercise it with a
--        prisoner_letter module_key value) ────────────────────────
DO $$
DECLARE
  v_org UUID;
  v_user UUID;
  ...
  v_letter UUID;
BEGIN
  SELECT id INTO v_org FROM organizations WHERE type = 'mcs' LIMIT 1;
  ...
  SELECT id INTO v_letter FROM prisoner_letters WHERE from_prison_id = v_org LIMIT 1;

  IF v_org IS NULL OR v_user IS NULL OR v_task IS NULL OR v_letter IS NULL THEN
    RAISE NOTICE 'Skipping active-link-uniqueness smoke test: no fixture org/user/task/letter row available in this database.';
    RETURN;
  END IF;

  BEGIN
    INSERT INTO task_links (task_id, module_key, record_id, organization_id, created_by) VALUES (v_task, 'prisoner_letter', v_letter, v_org, v_user);
  EXCEPTION
  ...
  BEGIN
    INSERT INTO task_links (task_id, module_key, record_id, organization_id, created_by) VALUES (v_task, 'prisoner_letter', v_letter, v_org, v_user);
    RAISE EXCEPTION 'VALIDATION_FAILED: a duplicate ACTIVE task_links row (module_key=prisoner_letter) was accepted';
  EXCEPTION
    WHEN unique_violation THEN
      RAISE NOTICE 'idx_task_links_active_unique correctly rejects a duplicate active prisoner_letter link';
  END;

  DELETE FROM task_links WHERE task_id = v_task AND module_key = 'prisoner_letter' AND record_id = v_letter AND created_by = v_user;
END $$;
...
DECLARE
  v_missing TEXT := '';
BEGIN
  IF NOT (SELECT pg_get_constraintdef(oid) LIKE '%''prisoner_letter''%' FROM pg_constraint WHERE conrelid = 'task_links'::regclass AND contype = 'c' AND conname LIKE '%module_key%') THEN
    v_missing := v_missing || 'constraint:task_links_module_key_check-missing-prisoner_letter ';
  END IF;
  ...
  IF NOT (SELECT pg_get_constraintdef(oid) LIKE '%''external_correspondence''%' FROM pg_constraint WHERE conrelid = 'task_links'::regclass AND contype = 'c' AND conname LIKE '%module_key%') THEN
    v_missing := v_missing || 'constraint:task_links_module_key_check-lost-external_correspondence ';
  END IF;
  ...
  IF NOT (SELECT pg_get_functiondef(oid) LIKE '%can_view_prisoner_letter%' FROM pg_proc WHERE proname = 'can_view_task_link') THEN
    v_missing := v_missing || 'function:can_view_task_link-missing-prisoner_letter-branch ';
  END IF;
  IF NOT (SELECT pg_get_functiondef(oid) LIKE '%can_view_entry%' FROM pg_proc WHERE proname = 'can_view_task_link') THEN
    v_missing := v_missing || 'function:can_view_task_link-lost-entry-branch ';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE p.proname IN (
      'can_view_prisoner_letter', 'can_manage_prisoner_letter_task_link',
      'create_prisoner_letter_supporting_task', 'link_existing_task_to_prisoner_letter',
      'unlink_task_from_prisoner_letter', 'get_prisoner_letter_task_capabilities',
      'can_view_task_link'
    )
    AND NOT EXISTS (
      SELECT 1 FROM unnest(COALESCE(p.proconfig, ARRAY[]::text[])) cfg WHERE cfg LIKE 'search_path=%'
  ...
  IF v_missing <> '' THEN
    RAISE EXCEPTION 'Prisoner-Letter-Task Integration validation FAILED, missing: %', v_missing USING ERRCODE = 'P0001';
  ELSE
    RAISE NOTICE 'Prisoner-Letter-Task Integration validation PASSED: constraints, indexes, RLS, helpers, and RPCs all present, search_path pinned throughout, request/meeting/internal_request/external_correspondence branches preserved.';
  END IF;
END $$;
