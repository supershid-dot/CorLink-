-- CAP-002 Phase 5.2 rollback: delegation/substitution runtime
-- integration.
--
-- Restores workflow_resolve_approval_candidates(), workflow_enter_
-- downstream_node(), and decide_workflow_work_item() to their exact
-- pre-5.2 bodies (byte-for-byte, extracted from the already-approved
-- patch-workflow-approval-round-lifecycle.sql for the first,
-- patch-workflow-gateway-routing-execution.sql for the second, and
-- patch-workflow-decision-replay-hardening.sql for the third — the
-- true current bodies immediately before this milestone, not any
-- earlier superseded version), and drops the two new private helper
-- functions this milestone added
-- (workflow_resolve_effective_candidate, workflow_resolve_active_
-- delegation).
--
-- Never refuses. This milestone is a pure function-body integration —
-- zero new tables, zero new columns, zero schema change of any kind
-- (workflow_approval_positions.section_id already existed since
-- Phase 2B.2; this milestone only started populating it). Rolling
-- back therefore risks no data: every workflow_decisions/
-- workflow_events/workflow_approval_positions row already written
-- (including any that happen to carry 'delegation_id'/'section_id'
-- values from this milestone's runtime) remains exactly as written —
-- workflow_events and workflow_decisions are already immutable/
-- append-only regardless of which function body wrote them, and
-- workflow_approval_positions.section_id is just a column value, not
-- a construct only the post-5.2 body can interpret. Rolling back only
-- changes what FUTURE candidate resolution/decision calls do, exactly
-- matching the Phase 4.3 rollback's "changes only function bodies, no
-- meaningful data at risk" precedent this rollback follows.
--
-- Phase 5.1's own delegation/substitution tables, RPCs, and data are
-- entirely untouched by this rollback — this milestone never modified
-- any of them, and rolling back the runtime integration does not
-- remove the persistence foundation underneath it.
\set ON_ERROR_STOP on
BEGIN;

-- ── 1. workflow_resolve_approval_candidates — restored to its exact
...
-- ── 2. workflow_enter_downstream_node — restored to its exact
--    pre-5.2 (Phase 4.2) body. ──────────────────────────────────────
...
-- ── 3. decide_workflow_work_item — restored to its exact pre-5.2
--    (Phase 4.3) body. ──────────────────────────────────────────────
...
-- ── 4. Drop the two new private helper functions this milestone
--    added. Nothing else references them once the three functions
--    above are restored. ────────────────────────────────────────────
DROP FUNCTION IF EXISTS workflow_resolve_effective_candidate(UUID,UUID,TEXT,UUID,UUID,TIMESTAMPTZ);
DROP FUNCTION IF EXISTS workflow_resolve_active_delegation(UUID,UUID,UUID,UUID,UUID,TEXT,TEXT,UUID,UUID,TIMESTAMPTZ);
