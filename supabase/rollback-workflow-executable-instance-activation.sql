-- CAP-002 Phase 2B.2 transactional rollback.
--
-- Removes only what this patch added/changed: restores
-- workflow_transition_instance() to its exact pre-2B.2 body (copied
-- verbatim from supabase/patch-workflow-runtime.sql — that file was
-- never edited by this patch, so its text is still the authoritative
-- pre-patch source, not hand-transcribed), and drops the two new
-- SELECT-only tables (workflow_approval_rounds, workflow_approval_positions).
-- The 5 public wrapper RPCs (start/suspend/resume/cancel/complete_
-- workflow_instance) were never touched by this patch and are left
-- alone; CREATE OR REPLACE on workflow_transition_instance preserves
-- its existing REVOKE/no-grant posture automatically.
--
-- Refuses to run if any workflow_approval_rounds or
-- workflow_approval_positions row exists, i.e. if any instance has
-- ever been activated through an approval node via this patch's
-- logic. Dropping those tables would irreversibly destroy that
-- round/position history, and docs/60-64 define no compatibility
-- path for representing it after rollback — this is an unconditional
-- refusal, not a soft warning. Instances that only ever reached the
-- Start->End special case leave no round/position rows (nothing to
-- lose there) and are otherwise inert once 'completed', so they do
-- not block rollback.
-- Preserves every Phase 1/2/2B.1 table, row, RLS policy, grant, and
-- index, and every workflow_instances/workflow_instance_steps/
-- workflow_tokens/workflow_work_items/workflow_events row already
-- written (including those written by activation before rollback).
\set ON_ERROR_STOP on
BEGIN;

DO $$
DECLARE v_rounds INTEGER; v_positions INTEGER;
BEGIN
  SELECT count(*) INTO v_rounds FROM workflow_approval_rounds;
  SELECT count(*) INTO v_positions FROM workflow_approval_positions;
  IF v_rounds > 0 OR v_positions > 0 THEN
    RAISE EXCEPTION 'Refusing rollback: % workflow_approval_rounds row(s) and % workflow_approval_positions row(s) exist. Rollback would drop the tables holding that activation history with no compatibility path defined.', v_rounds, v_positions
      USING ERRCODE = '55000';
  END IF;
END $$;
...
DROP TABLE IF EXISTS workflow_approval_positions;
DROP TABLE IF EXISTS workflow_approval_rounds;
