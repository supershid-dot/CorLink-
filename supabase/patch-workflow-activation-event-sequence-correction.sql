-- ============================================================
-- CAP-002 Phase 2B.2A: Workflow Activation Event Sequence Correction
--
-- Corrects ONLY a next_event_sequence arithmetic defect in
-- workflow_transition_instance()'s Approval-branch, discovered while
-- building Phase 2C.1 (since suspended, no Phase 2C code remains).
--
-- Root cause: the approval branch's next_event_sequence UPDATE used
--   next_event_sequence + 5 + electorate_count + LEAST(electorate_count, offered_count)
-- The correct increment is the exact count of events this branch
-- actually inserts: 7 fixed events (instance_started, token_created,
-- step_entered[start], step_completed[start], token_moved,
-- step_entered[target], approval_round_opened) plus one
-- work_item_created per OFFERED position:
--   next_event_sequence + 7 + LEAST(electorate_count, offered_count)
-- These two formulas coincide only when electorate_count = 2 — every
-- Phase 2B.1/2B.2 test fixture used exactly 2 candidates, so the
-- defect passed all existing tests undetected. For electorate_count
-- other than 2, next_event_sequence was left either too low (electorate
-- < 2: the very next event-writing command on that instance collides
-- with an already-used sequence number and crashes with a unique-
-- constraint violation) or too high (electorate > 2: sequence numbers
-- are silently skipped, a permanent gap in the event ledger).
--
-- This patch changes ONLY that one arithmetic expression. No event is
-- added, removed, or reordered; no runtime semantics, approval
-- behavior, authorization, locking, idempotency, or version-pinning
-- logic changes. The function is reproduced here in full (required
-- for CREATE OR REPLACE) but is byte-for-byte identical to the
-- approved Phase 2B.2 body (supabase/patch-workflow-executable-
-- instance-activation.sql) except for this single line.
-- ============================================================
  ...
  IF v_is_executable THEN
    ...
    ELSIF v_target_type = 'approval' THEN
      ...
      SET status = 'active', started_at = COALESCE(started_at, v_now),
          lock_version = v_result_lock_version,
          next_event_sequence = next_event_sequence + 7 + LEAST(v_electorate_count, CASE WHEN v_delivery_mode = 'parallel' THEN v_electorate_count ELSE 1 END)
      WHERE id = p_instance_id;
